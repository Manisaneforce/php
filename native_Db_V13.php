...
function getOrderDetByDt(){
	...
	if($StrMode == "VstRet" || $StrMode == "VstPRet"){
		...
		s.Stockist_Name StkName,s.Stockist_Address StkAddr,l.ListedDr_Mobile StkMob,h.Trans_Sl_No,iif(OrderType=0,'Field Order','Phone Order')  from Trans_Order_Head h
		...
		isnull(s.S_Name,'') StkName,isnull(s.Contact_Person,'') StkAddr,isnull(s.Mobile,'') StkMob,h.Trans_Sl_No,iif(OrderType=0,'Field Order','Phone Order')  from Trans_PriOrder_Head h
...
$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types";
    ...
    case "get/precall":
        ...
        $SfCode = $_GET['SfCode'];
        $RetailerCode = $_GET['RetailerCode'];
        $sql = "";
        $result = performQuery($sql);
        outputJSON($result);
    break;
...
break;
	...
	break;
	 case "get/travel_approval":
            $sf_code = $_GET['sf_code'];
            $date = $_GET['date'];
            $query = "select * from Expense_Start_Activity where sf_code = '$sf_code' and CONVERT(date,ExpenseDate) = '$date' and isNull(Approval_Flag,0)<>1";
            $result = performQuery($query);
            outputJSON($result);
    break; 
